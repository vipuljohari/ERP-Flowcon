...
const Inventory: React.FC<InventoryProps> = ({ 
  ...
  const commitOpeningBalance = (partId: string, currentOpening: number) => {
    ...
    const remark = `[OPENING_BALANCE_SET:${newVal}|PREV:${currentOpening}] Item Opening Balance set to ${newVal} Pcs from Previous ${currentOpening} Pcs (${deltaStr} Pcs) for ${part?.name || 'Item'} [${monthDisplay}]`;

    if (window.confirm(`Audit Action: Adjust Item Opening Balance?\n\nTarget Balance: ${newVal} Pcs\nPrevious Balance: ${currentOpening} Pcs\nCorrection: ${deltaStr} Pcs\n\nThis will record an audit entry to persist the stock level.`)) {
      onAddInward(partId, delta, "ADMIN_AUDIT", remark, actionTimestamp);
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredParts.map(p => {
                    ...
                    const receiptsSinceStartOfMonth = monthLogs
                      .filter(l => !l.remarks?.startsWith("[OPENING_BALANCE_SET:") && l.remarks !== "[OPENING_BALANCE_ADJUSTMENT]")
                    ...
                    } else if (adjustmentEntry) {
                      const match = adjustmentEntry.remarks?.match(/\[OPENING_BALANCE_SET:(\d+)\]/);
                      if (match) openingBalValue = parseInt(match[1]);
                      else openingBalValue = Math.round(p.stock - receiptsSinceStartOfMonth + totalSalesSinceStartOfMonth);
                    } else {
                      openingBalValue = Math.round(p.stock - receiptsSinceStartOfMonth + totalSalesSinceStartOfMonth);
                    }
                    ...
                    {filteredParts.map(p => {
                      ...
                      if (mappedRMs.length > 0) {
                        openingBalValue = mappedRMs.reduce((sum, rm) => {
                          ...
                          const partLength = p.itemLength || 0;
                          let yieldFactor = 0;
                          if (partLength > 0) yieldFactor = Math.floor(rmLength / partLength);
                          else {
                            const rmWeight = rmLength * (rm.weightPer1000 / 1000);
                            const partWeight = p.itemWeight || 0;
                            if (partWeight > 0 && rmWeight > 0) yieldFactor = Math.floor(rmWeight / partWeight);
